...
Searching: The main finding function is:
    ...
    void find(int64_t value, size_t start, size_t end, size_t baseindex, QueryStateBase *state) const
    cond:       One of Equal, NotEqual, Greater, etc. classes

    find() will call state->match() on each row between start and end. If match()
    returns false then find() will exit.
*/
                                                       ...
                                                       QueryStateBase* state) const
{
    ...
    for (; start2 < end2; start2++)
        if (!state->match(start2 + baseindex, m_array.get<bitwidth>(start2)))
            return false;
    return true;
}

// This is the main finding function for Array. Other finding functions are just wrappers around this one.
// Search for 'value' using condition cond (Equal, NotEqual, Less, etc) and call match(). Break and return
// if match() returns false or 'end' is reached.
template <class cond, size_t bitwidth>
...
bool ArrayWithFind::find_gtlt_fast(uint64_t chunk, uint64_t magic, QueryStateBase* state, size_t baseindex) const
{
    ...
    while (m) {
        ...
        p += t;
        if (!state->match(p + baseindex, int64_t((chunk >> (p * width)) & mask1)))
            return false;
                                            ...
                                            QueryStateBase* state) const
{
    ...
    for (; start < ee; ++start)
        if (eq ? (m_array.get<width>(start) == value) : (m_array.get<width>(start) != value)) {
            if (!state->match(start + baseindex, m_array.get<width>(start)))
                return false;
        }
    ...
    if (width != 32 && width != 64) {
        ...
        while (p < e) {
            ...
            while (eq ? test_zero<width>(v2) : v2) {
                ...
                if (!state->match(a + start + baseindex, m_array.get<width>(start + a)))
                    return false;
                                                      ...
                                                      QueryStateBase* state, size_t baseindex) const
{
    ...
    for (i = 0; i < items; ++i) {
        ...
        while (resmask != 0) {
            ...
            s += idx;
            if (!state->match(s + baseindex, m_array.get_universal<width>(reinterpret_cast<char*>(action_data), s)))
                return false;
